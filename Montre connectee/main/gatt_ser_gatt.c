...
esp_attr_value_t gatts_demo_char1_val =
{
	...
	.attr_len		= sizeof(prof_shared_buf),
	.attr_value     = (uint8_t *)prof_shared_buf,
};
...
void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        //char temp_tab[5];//iciii*************yasmine**********************************
        //printf( "temperature:%s",temp_tab );
        //***************************
       /* char temp_tab[5]; // Assurez-vous que la taille est suffisante pour contenir la chaîne de caractères
        snprintf(temp_tab, sizeof(temp_tab), "%.2f", temperature_degC);*/

        // Imprimer la chaîne de caractères pour vérifier
        ...
        //*********
        /*rsp.attr_value.value[0] = temp_tab[0];
        rsp.attr_value.value[1] = temp_tab[1];
        rsp.attr_value.value[2] = temp_tab[2];
        rsp.attr_value.value[3] = temp_tab[3];
        rsp.attr_value.value[4] = temp_tab[4];*/
...
void float_to_char_array(float value, char *result) {
    // Convert the float to an integer by multiplying with a power of 10
    int integer_part = (int)value;
    int decimal_part = (int)((value - integer_part) * 100); // Extracting 2 decimal places

    // Extract individual digits
    result[0] = integer_part / 10 + '0';
    result[1] = integer_part % 10 + '0';
    result[2] = '.';
    result[3] = decimal_part / 10 + '0';
    result[4] = decimal_part % 10 + '0';
